...
import { 
  ...
  RiMapPinLine, RiAddLine, RiCloseLine, RiFileTextLine
} from 'react-icons/ri';

const Settings = () => {
  const { data, updateData } = useData();
  const [formData, setFormData] = useState<any>(null);
  const [activeTab, setActiveTab] = useState('about'); // Varsayılan sekme Hakkımızda olsun
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateData('about', formData.about); // Hakkımızda Kaydı
    ...
    <AdminLayout>
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400">Site içeriklerini buradan yönetin.</p>
        </div>
      ...
      <div className="flex gap-4 border-b border-slate-700 mb-8 overflow-x-auto pb-2 scrollbar-hide">
        {['about', 'prices', 'general', 'contact', 'footer'].map(tab => (
      ...
      <div className="max-w-4xl animate-fadeIn">
        ...
        {/* HAKKIMIZDA & BÖLGELER SEKMESİ (BİRLEŞTİRİLDİ) */}
        {activeTab === 'about' && (
          <div className="space-y-6">
            
            {/* Hakkımızda Alanı */}
            <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><RiFileTextLine className="text-yellow-500"/> Hakkımızda Yazısı</h3>
              
              <label className="text-slate-400 text-sm block mb-2">Başlık</label>
              <input type="text" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white mb-4"
                value={formData.about?.title || ''} 
                onChange={e => setFormData({...formData, about: {...formData.about, title: e.target.value}})}
              />

              <label className="text-slate-400 text-sm block mb-2">İçerik (Paragraf)</label>
              <textarea rows={6} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white"
            ...
            {/* Bölgeler Alanı */}
            <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><RiMapPinLine className="text-orange-500"/> Hizmet Bölgeleri</h3>
              <div className="flex gap-2 mb-4">
                <input type="text" className="flex-1 bg-slate-900 border border-slate-700 rounded-lg p-3 text-white" placeholder="Yeni Bölge Ekle (Mahalle/İlçe)"
                  value={newRegion} onChange={e => setNewRegion(e.target.value)} onKeyPress={e => e.key === 'Enter' && addRegion()}
                />
                <button onClick={addRegion} className="bg-green-600 text-white px-4 rounded-lg font-bold"><RiAddLine/></button>
              </div>
              <div className="flex flex-wrap gap-2">
                {formData.regions?.map((r: string, i: number) => (
                  <span key={i} className="bg-slate-800 border border-slate-600 px-3 py-1 rounded-full text-sm flex items-center gap-2 text-slate-200">
                    {r} <button onClick={() => removeRegion(i)} className="text-red-400 hover:text-red-300"><RiCloseLine/></button>
                  </span>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'prices' && (
          <div className="space-y-6">
            <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700">
               <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><RiHomeGearLine className="text-purple-500"/> Oda Fiyat Aralıkları</h3>
               <p className="text-slate-400 text-sm mb-4">Müşteriye gösterilecek tahmini fiyat aralıkları.</p>
               ...
               <div className="space-y-3">
                 ...
                 {['1+0','2+0','1+1','2+1','3+1','4+1','5+1','6+1'].map(id => (
                    <RoomRangeInput key={id} label={`${id} Daire`} id={id} data={formData} set={setFormData} />
                 ))}
               </div>
            </div>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white mt-1"
                    value={formData.prices.kmPrice} onChange={e => setFormData({...formData, prices: {...formData.prices, kmPrice: Number(e.target.value)}})}
                  />
                </div>
                <div>
                  <label className="text-slate-400 text-sm">Asansörlü Kat (₺)</label>
                  <input type="number" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white mt-1"
                    value={formData.prices.elevatorCost} onChange={e => setFormData({...formData, prices: {...formData.prices, elevatorCost: Number(e.target.value)}})}
                  />
                </div>
                <div>
                  <label className="text-slate-400 text-sm">Merdivenli Kat (₺)</label>
                  <input type="number" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white mt-1"
                    value={formData.prices.stairsCost} onChange={e => setFormData({...formData, prices: {...formData.prices, stairsCost: Number(e.target.value)}})}
                  />
                </div>
              </div>
            </div>
        ...
        {/* DİĞER SEKMELER (Sadece içeriği gösteriyorum, mantık aynı) */}
        {activeTab === 'general' && (
             <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700">
